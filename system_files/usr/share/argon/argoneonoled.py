...
def oled_loadbg(bgname):
	if bgname == "bgblack":
		...
		return
	try:
		file = open("/usr/share/argon/oled/"+bgname+".bin", "rb")
...
def oled_writetext(textdata, x, y, charwd = 6, mode = 0):
	...
	try:
		file = open("/usr/share/argon/oled/font"+str(charht)+"x"+str(charwd)+".bin", "rb")
		fontbytes = list(file.read())
		file.close()
	except FileNotFoundError:
		try:
			# Default to smallest
			file = open("/usr/share/argon/oled/font8x6.bin", "rb")
			fontbytes = list(file.read())
			file.close()
		except FileNotFoundError:
			return
